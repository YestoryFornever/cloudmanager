...
@Component({
	selector: 'customer-manage-serch.component',
	templateUrl: './customer-manage-serch.component.html',
	styleUrls: ['./customer-manage-serch.component.css'],
	providers: [CustomerManageService]
})
export class CustomerManageSerchComponent implements OnInit{
...
// 筛选条件  
	public typeaheadOnCompanys(e:TypeaheadMatch):void{
		// this.default.organizationShortName = e.item.organizationShortName;//id 赋值给隐藏的input。获取公司id 
		...
		let listState= <HTMLInputElement[]><any>document.getElementsByName("user");				
			for(var i =0;i<listState.length;i++){
		 		if(listState[i].checked == true){
		 			this.orgIdList.push( this.customerList[i].organizationId );
		 			this.orgIdNum++
				}
			}
...
// 类管理
	...
	// 获取已分配机构
	customerList:any
	queryAllocatedOrgList(){
		this.default.empId = this.id,
		this.default.organizationCategoryId1=this.orgCondition.c1
		this.default.organizationCategoryId2=this.orgCondition.c2
		 this.customerManageService.queryAllocatedOrgList(this.default) 
	        .subscribe(
	            customerList => {
	            	console.log(customerList)
	                if(customerList.status==0){
	                    this.customerList=customerList.data.voList;
	                    this.totalItems = customerList.data.total
	                    if(!customerList.data.total){
	                    	 this.totalItems=0
	                    }
	                }else{
	                	if(customerList.msg){
	...
	removeOrgList(abandon:any){
		...
		console.log(obj)
		this.customerManageService.removeOrgList(obj) 
	        .subscribe(
	            customerList => {
	            	console.log(customerList)
	                if(customerList.status==0){
	...
	// 跳转
	toChange(){
	   this.router.navigate(['../../../../customer-manage-allocation',this.id,this.uName,this.lName],{relativeTo:this.activatedRoute}); 
	}
	reTurn(){
