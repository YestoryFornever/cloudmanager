...
//审核弹窗
	...
	public typeaheadOnCompanysVer(e:TypeaheadMatch){
...
//获取公司
	postListMsges(){
    this.userApproveService.OrganizationList(null) 
        .subscribe(
            companylist => {
...
//变更信息
	auditRealChange(x:number,windows:any){
		var obj = {
			...
			cardAlterId:this.msgWindow.cardAlterId,	
			userId:	this.msgWindow.userId,
			organizationId:	this.organizationIdChange1,
			remark:this.reMark,
			visitState:this.radioModel,
			updateField:this.msgWindow.updateField,
			updateValue:this.msgWindow.updateValue,
			auditState:x
		}
		if(this.reMark==undefined){
			this.reMark=''
		...
		if(obj.auditState==2&&this.msgWindow.updateField=='机构简称'&& obj.organizationId==''){
			alert("请填写机构简称")
			return false
		}
		this.userApproveService.approveVer(obj)
