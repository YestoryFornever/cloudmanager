...
@Component({
	...
	styleUrls: ['./sns-news-change.component.css'],
	...
	getInfoDetails(){
		...
		this.snsManagementService.getinfoContent(this.ID)
								...
								detial => {
									...
									this.detial.source_time=this.detial.source_time.replace(' ','T')		
...
infoUpdate(save:any){
	...
	this.default.source_time = this.detial.source_time.replace('T',' ');
	this.default.source = this.detial.source
	if(this.sc2=="2"){
		this.default.category+="2,"
	}
	if(this.sc4=="4"){
