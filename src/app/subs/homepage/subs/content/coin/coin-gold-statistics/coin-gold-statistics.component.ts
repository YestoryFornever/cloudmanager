...
import { CoinGoldStatisticsService } from './coin-gold-statistics.services/coin-gold-statistics.services';
@Component({
	selector: 'coin-gold-statistics',
	templateUrl: './coin-gold-statistics.component.html',
	styleUrls: ['./coin-gold-statistics.component.css'],
	providers: [CoinGoldStatisticsService]
})
export class CoinGoldStatisticsComponent {
	constructor(
		private coinGoldStatisticsService:CoinGoldStatisticsService
	){}
	...
	errorMessage:any;
	 ...
	 myAlert:any;//刷新按钮开关
	coinstatistics(){
    	this.coinGoldStatisticsService.coinstatistics(null) 
        .subscribe(
            coinstatistics => {
            	// debugger
                if(coinstatistics.status==0){
                    ...
                    this.todyDeduct  =  coinstatistics.data.cancelRealName + coinstatistics.data.cancelRecommend;// 实名认证奖励扣除 + 推荐用户奖励扣除
	...
	todaystatistics(){
    	this.coinGoldStatisticsService.todaystatistics(null) 
        .subscribe(
            todaystatistics => {
